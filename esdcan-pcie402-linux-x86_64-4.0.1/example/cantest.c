...
/*                                                                      */
/*          Copyright 1997 - 2018 by esd electronics gmbh               */
/*----------------------------------------------------------------------*/
/*                                                                      */
/*      Filename:      cantest.c                                        */
/*      Date:          1999-11-09                                       */
/*      Language:      ANSI C                                           */
/*      Targetsystem:  Win 10/8/7/Vista/XP/2K/NT, Win 9x/ME, WinCE, QNX */
/*                     UNIX, VxWorks, Linux (RTAI/RT), NET+OS, RTOS-UH, */
/*                     RTX/RTX64, OnTime RTOS-32                        */
/*                                                                      */
/*      Purpose:       Test and demonstration for CAN device drivers    */
/*                     which support the NTCAN-API                      */
/*----------------------------------------------------------------------*/
/* Revision history:                                                    */
/*----------------------------------------------------------------------*/
/* 30A,27aug18,mk  * Fixed 2nd FW version output                        */
/* 309,22aug18,stm * Make compilable again without NTCAN_IOCTL_GET_INFO.*/
/*                 * Fixed output of open handles for interfaces without*/
/*                 * working NTCAN_IOCTL_GET_INFO call by moving ')'.   */
/* 308,31jul18,stm * Don't call NTCAN_IOCTL_GET_FD_TDC on interfaces not*/
/*                 *  CAN-FD capable and don't display it!              */
/* 307,31jan18,ot  * Send reference frame for tests 20/21 and 60/61     */
/*                 *  to validate initial Tx delay                      */
/* 306,14dec17,stm * Made compilable again with Linux non CAN-FD libs.  */
/* 305,14dec17,ot  * Added new transceiver types                        */
/* 304,10aug17,ot  * Corrected meta data ASCII interpretation for       */
/*                 *  object mode handle.                               */
/*                 * Added missing indication of data bitrate in        */
/*                 *  initial overview message                          */
/* 303,11mar17,ot  * Fixed compiler errors if compiled with legacy      */
/*                 *  header without NTCAN_FD definition                */
/* 302,08dec16,ot  * Wait before closing a handle for all non-blocking  */
/*                 *  transmissions and not just for test 0.            */
/*                 * Support numerical values for bit rates.            */
/*                 * Validate bit rate configuration for CAN FD tests.  */
/* 301,14nov16,mk  * Fixed format strings for object mode frame print   */
/* 300,11oct16,ot  * Simplified setting a (FD data) bit rate            */
/*                 * Message display completely revised for CAN FD      */
/*                 * Added support to send CAN FD frames without BRS    */
/*                 * Evaluate new FD related member of bus statistic    */
/*                 * Evaluate new FD related member of bit rate details */
/*                 * Decode feature Triple Sampling and CAN FD TDC      */
/*                 * Support for data bit rate change event             */
/*                 * Show FD bit rates if called as test -4             */
/* 2C7,23feb16,ot  * Added test 120 to measure system Tx delay          */
...
static char* rcsid __attribute__((unused)) = "$Id: cantest.c 16389 2018-08-31 16:39:50Z stefanm $";
#else  /* No or old GNU compiler */
# define USE(var) static void use_##var(void *x) {if(x) use_##var((void *)var);}
static char* rcsid = "$Id: cantest.c 16389 2018-08-31 16:39:50Z stefanm $";
USE(rcsid);
 ...
 */
/*
 * Array of strings with meta data flags based on Bit 4..7 of the len field
 * where the reserved bit 6 is replaced with Bit 29 of the identifier.
 *
 *   B = Bit Rate Switch (CAN FD only)
 *   E = Extended Frame Format
 *   F = CAN FD frame
 *   I = Interaction
 *   R = RTR Frame (Classical CAN only)
 */
static const char *pNtcanMetaData[16] = {
    "    ", "   R", "  I ", "  IR",
    " E  ", " E R", " EI ", " EIR",
    "F  B", "F   ", "F IB", "F I ",
    "FE B", "FE  ", "FEIB", "FEI ",
...
PFN_CAN_SET_BAUDRATE_X          pfnSetBaudrateX         = NULL;
PFN_CAN_GET_BAUDRATE_X          pfnGetBaudrateX         = NULL;
...
#  define canSetBaudrateX(hnd, pBaud) \
    (NULL == pfnSetBaudrateX) ? NTCAN_NOT_IMPLEMENTED : \
    pfnSetBaudrateX((hnd), (pBaud))
#  define canGetBaudrateX(hnd, pBaud) \
    (NULL == pfnGetBaudrateX) ? NTCAN_NOT_IMPLEMENTED : \
    pfnGetBaudrateX((hnd), (pBaud))
...
/*
 * Parse the bit rate argument.
 *
 * Supported special values are:
 *  "auto"    - Start automatic bit rate detection (Classical CAN only)
 *  "disable" - Remove from bus
 *  "no"      - Do not change the configured bit rate.
 *
 * If none of the above applies the argument is expected to be in the
 * format <baud_nom> (Classical CAN) or <baud_nom>:<baud_data> (CAN FD).
 * <baud_nom> can be a numerical value or one of the special strings
 *            10K, 20K, 50K, 100K, 125K, 250K, 500K, 1M
 * <baud_data> can be a numerical value or one of the special strings
 *            500K, 1M, 2M, 4M, 5M, 8M, 10M
 */
static int parse_baud_arg(char *pArg, uint32_t *pBaudNominal, uint32_t *pBaudData)
{
    static const char *cShortNominal[NTCAN_BAUD_800 + 1] =
        {"1M","","500K","","250K","","125K","100K",
         "","50K","","20K","","10K","800K"};
#ifdef NTCAN_FD
    static const char *cShortData[NTCAN_BAUD_10000 + 1] =
        {"1M","","500K","","","","","","","","","","",
         "","","","","2M","4M","5M","8M","10M"};
#endif
    char *pEnd;
    ...
    *pBaudData    = 0xFFFFFFFF;
    *pBaudNominal = 0xFFFFFFFF;

    if(!strcmp(pArg, "auto")) {
        *pBaudNominal = NTCAN_AUTOBAUD;
    } else if(!strcmp(pArg, "disable")) {
        *pBaudNominal = NTCAN_NO_BAUDRATE;
    } else if(!strcmp(pArg, "no")) {
        *pBaudNominal = 0xFFFFFFFFU;
    } else {
#ifdef NTCAN_FD
        pArg = strtok(pArg, ":");
        if(NULL == pArg) {
...
#endif
        for(i = 0; i < (int)(sizeof(cShortNominal) / sizeof(cShortNominal[0])); i++) {
            if(!strcmp(pArg, cShortNominal[i])) {
                *pBaudNominal = i;
                break;
            }
        }
        if(0xFFFFFFFF == *pBaudNominal) {
            *pBaudNominal = strtoul(pArg, &pEnd, 0);
            if (*pEnd != '\0') {
                return(-1);
            }
        }
        ...
        pArg = strtok(NULL, ":");
        if(pArg != NULL) {
            for(i = 0; i < (int)(sizeof(cShortData) / sizeof(cShortData[0])); i++) {
                if(!strcmp(pArg, cShortData[i])) {
                    *pBaudData = i;
                    break;
                }
            }
            if(0xFFFFFFFF == *pBaudData) {
                *pBaudData = strtoul(pArg, &pEnd, 0);
                if (*pEnd != '\0') {
                    return(-1);
                }
            }
...
#ifdef NTCAN_FD
/*
 * Initialize the NTCAN_BAUDRATE_X structure based on the given nominal
 * and data phase bit rate.
 */
static int baudx_initialize(NTCAN_BAUDRATE_X *pBaud, uint32_t baudNominal,
                            uint32_t baudData)
{
    memset(pBaud, 0, sizeof(NTCAN_BAUDRATE_X));

    /* Setup the mode field */
    if(NTCAN_NO_BAUDRATE == baudNominal) {
        pBaud->mode = NTCAN_BAUDRATE_MODE_DISABLE;
        return(0);
    } else if(NTCAN_AUTOBAUD == baudNominal) {
        pBaud->mode = NTCAN_BAUDRATE_MODE_AUTOBAUD;
        return(0);
    } else if(baudNominal & NTCAN_USER_BAUDRATE_NUM) {
        pBaud->mode = NTCAN_BAUDRATE_MODE_NUM;
        pBaud->arb.u.rate  = baudNominal & 0x0FFFFFFF;
        pBaud->data.u.rate = baudData;
    } else if(baudNominal & NTCAN_USER_BAUDRATE) {
        pBaud->mode = NTCAN_BAUDRATE_MODE_BTR_CANONICAL;
        pBaud->arb.u.btr.tseg1  = (uint16_t)baudNominal & 0xFF;
        pBaud->arb.u.btr.tseg2  = (uint16_t)(baudNominal >> 8) & 0xFF;
        pBaud->arb.u.btr.sjw    = (uint16_t)(baudNominal >> 16) & 0x7F;
        pBaud->arb.u.btr.brp    = (uint16_t)(baudNominal >> 23) & 0x1F;
        pBaud->data.u.btr.tseg1 = (uint16_t)baudData & 0xFF;
        pBaud->data.u.btr.tseg2 = (uint16_t)(baudData >> 8) & 0xFF;
        pBaud->data.u.btr.sjw   = (uint16_t)(baudData >> 16) & 0x7F;
        pBaud->data.u.btr.brp   = (uint16_t)(baudData >> 23) & 0x1F;
    } else {
        if(baudNominal > 0xFF) {
            pBaud->mode = NTCAN_BAUDRATE_MODE_NUM;
            pBaud->arb.u.rate  = baudNominal & 0x0FFFFFFF;
            pBaud->data.u.rate = baudData & 0xFFFFFFF;
        } else {
        pBaud->mode = NTCAN_BAUDRATE_MODE_INDEX;
        pBaud->arb.u.idx  = baudNominal & 0x1F;
        pBaud->data.u.idx = baudData & 0x1F;
    }
    }

    /* Setup the flag field */
    if(baudNominal & NTCAN_LISTEN_ONLY_MODE) {
        pBaud->flags |= NTCAN_BAUDRATE_FLAG_LOM;
    } else if (baudNominal & NTCAN_SELF_TEST_MODE) {
        pBaud->flags |= NTCAN_BAUDRATE_FLAG_STM;
    }

    if(baudData != 0xFFFFFFFF) {
        pBaud->flags |= NTCAN_BAUDRATE_FLAG_FD;
    }
...
# endif /* of _WIN32_WCE */
  ...
  if (argc > 5) {       /* Parameter 'count' */
     ...
     * frames are sent instead of data frames for Classical CAN frames or
     * without bit rate switch for CAN FD frames.
     */
    if(maxcount < 0) {
    ...
    if (IS_SEND_TEST(test) || IS_WRITE_TEST(test) || (5 == test)) {
      ...
      if (maxcount > MAX_RX_MSG_CNT && test != 124) {
  ...
  if (argc > 10) {           /* Parameter 'baudrate' */
      if(parse_baud_arg(argv[10], &baudrate, &baudData) != 0) {
  ...
  for (i = 0; i < MAX_TX_MSG_CNT; i++) {
    ...
    *((uint32_t*)(&cmt->data[4])) = k;
    ...
    cmx = &pCtx->txmsg_x[i];
    cmx->id = idstart;
    cmx->len = (uint8_t)(NTCAN_FD | NTCAN_DATASIZE_TO_DLC(64) | rtr_or_nobrs);
  ...
  if (120 == test) {
      idend       = idstart;
      pCtx->mode |= NTCAN_MODE_LOCAL_ECHO;
  }
  ...
  printf("count=%ld\ntxbuf=%ld rxbuf=%ld txtout=%ld rxtout=%ld ",
         (unsigned long)maxcount, (unsigned long)txbufsize, (unsigned long)rxbufsize,
         (unsigned long)txtout, (unsigned long)rxtout);
  if (baudrate == 0xFFFFFFFFU) {
    printf("baudrate=(no change)");
  } else if (baudrate >= NTCAN_BAUD_800) {
    printf("baudrate=0x%lx", (unsigned long)baudrate);
  } else {
    printf("baudrate=%ld", (unsigned long)baudrate);
  }
#if defined(NTCAN_FD)
  if (baudData != 0xFFFFFFFFU) {
      if (baudData >= NTCAN_BAUD_10000) {
          printf("/0x%lx", (unsigned long)baudData);
      } else {
          printf("/%ld", (unsigned long)baudData);
      }
  }
#endif
   ...
   */
#ifdef NTCAN_IOCTL_GET_TIMESTAMP
  if ((23 == test)  || (22 == test) || (21 == test) || (20 == test) || (42 == test) || (100 == test) ||
      (16 == test)  || (63 == test) || (62 == test) || (61 == test) || (60 == test) || (82 == test)  ||
      (110 == test) || (66 == test) || (120 == test) || (124 == test) )
  {
...
#ifdef NTCAN_FD
      if((pCtx->mode & NTCAN_MODE_FD) != 0) {
          baudx_initialize(&baudX, baudrate, baudData);
          ret = canSetBaudrateX(h0, &baudX);
          if (ret != NTCAN_SUCCESS) {
              printf("canSetBaudrateX() returned: %s\n", get_error_str(str_buf,ret));
              CANTEST_FREE(pCtx);
              return(-1);
          }
      } else {
#endif
          if((0 == (baudrate & (NTCAN_USER_BAUDRATE_NUM | NTCAN_USER_BAUDRATE))) &&
              ((baudrate & 0x0FFFFFFF) > 0xFF)) {
              baudrate |= NTCAN_USER_BAUDRATE_NUM;
          }
  ...
  */
  ...
  for (h = 0; (h < testcount) || ever; h++) {
    ...
    case 61:
      ...
      /* Send (Classical CAN) reference frame */
      {
          int32_t len2 = 1;
          pCtx->txmsg_t[0].data[4] = 0;
          ret = canSend(h0, (CMSG *)pCtx->txmsg_t, &len2);
          pCtx->txmsg_t[0].data[4] = 1;
          if (ret != NTCAN_SUCCESS) {
              printf("canSend returned: %s (sending reference frame)\n", get_error_str(str_buf, ret));
          }
      }
      ...
      /* Schedule first frame to be send in "TX-timeout" milliseconds */
      pCtx->txmsg_x[0].timestamp += ((pCtx->udtTimestampFreq * (uint64_t)txtout) / 1000ULL);
      for (i = 1; i < len; i++) {
	/* Every other frame will follow with "RX-timeout" milliseconds offset */
        pCtx->txmsg_x[i].timestamp = pCtx->txmsg_x[i-1].timestamp +
	                             ((pCtx->udtTimestampFreq * (uint64_t)rxtout) / 1000ULL);
      }
#endif
      /* Schedule first frame to be send in "TX-timeout" milliseconds */
      pCtx->txmsg_t[0].timestamp += ((pCtx->udtTimestampFreq * (uint64_t)txtout) / 1000ULL);
      for (i = 1; i < len; i++) {
	/* Every other frame will follow with "RX-timeout" milliseconds offset */
        pCtx->txmsg_t[i].timestamp = pCtx->txmsg_t[i-1].timestamp +
	                             ((pCtx->udtTimestampFreq * (uint64_t)rxtout) / 1000ULL);
      }
...
#ifdef NTCAN_FD
          printf("                : Type(Classic/FD): %ld/%ld\n",
              (long)(stat.rcv_count.std_data + stat.rcv_count.std_rtr +
                     stat.rcv_count.ext_data + stat.rcv_count.ext_rtr -
                     stat.rcv_count_fd), (long)stat.rcv_count_fd);
#endif
...
#ifdef NTCAN_FD
          printf("                : Type(Classic/FD): %ld/%ld\n",
              (long)(stat.xmit_count.std_data + stat.xmit_count.std_rtr +
                     stat.xmit_count.ext_data + stat.xmit_count.ext_rtr -
                     stat.xmit_count_fd), (long)stat.xmit_count_fd);
#endif
    ...
    case 84:
      {
          ...
          printf("CAN bitrate:\n------------\n");
#if defined(NTCAN_FD)
          if(NTCAN_BAUD_FD == bitrate.baud) {
            printf("Mode used for canSetBaudrateX()      : %d\n",
                  bitrate.mode);
          } else
          ...
          if (NTCAN_SUCCESS == bitrate.valid) {
              ...
              if(NTCAN_BAUD_FD == bitrate.baud) {
                  printf("Controller clock rate                : %ld.%ldMHz\n",
                      (long)(bitrate.clock/1000000), (long)(bitrate.clock%1000000));
                  printf("Nominal bit rate                     : %ld Bits/s\n",
                      (long)bitrate.rate);
                  tq = (long)(bitrate.tq_pre_sp + bitrate.tq_post_sp);
                  printf("Time quanta per Bit (NTQ)            : %ld\n",
                      tq);
                  printf("Pre-scaler (NBRP)                    : %ld\n",
                      (bitrate.clock / (bitrate.rate * tq)));
                  printf("Time quanta before SP (NTSEG1+SYNC)  : %ld\n",
                      (long)bitrate.tq_pre_sp);
                  printf("Time quanta after SP (NTSEG2)        : %ld\n",
                      (long)bitrate.tq_post_sp);
                  printf("Synchronization Jump Width (NSJW)    : %ld\n",
                      (long)bitrate.sjw);
                  sp = (long)((bitrate.tq_pre_sp * 10000) /
                      (bitrate.tq_pre_sp + bitrate.tq_post_sp));
                  printf("Position SP (NSP)                    : %ld.%ld%%\n",
                      sp/100, sp%100);
                  printf("Data bit rate                        : %ld Bits/s\n",
                      (long)bitrate.rate_d);
                  if(bitrate.rate_d != 0) {
                    tq = (long)(bitrate.tq_pre_sp_d + bitrate.tq_post_sp_d);
                    printf("Time quanta per Bit (DTQ)            : %ld\n",
                        tq);
                    printf("Pre-scaler (DBRP)                    : %ld\n",
                        (bitrate.clock / (bitrate.rate_d * tq)));
                    printf("Time quanta before SP (DTSEG1+SYNC)  : %ld\n",
                          (long)bitrate.tq_pre_sp_d);
                    printf("Time quanta after SP (DTSEG2)        : %ld\n",
                          (long)bitrate.tq_post_sp_d);
                    printf("Synchronization Jump Width (DSJW)    : %ld\n",
                          (long)bitrate.sjw_d);
                    sp = (long)((bitrate.tq_pre_sp_d * 10000) /
                          (bitrate.tq_pre_sp_d + bitrate.tq_post_sp_d));
                    printf("Position SP (DSP)                    : %ld.%ld%%\n",
                          sp/100, sp%100);
                  }
...
# endif /* of NTCAN_IOCTL_TX_OBJ_SCHEDULE */
    ...
    case 9:
       ...
       */
      if (!(pCtx->mode & NO_ASCII_OUTPUT)) {
        printf("Duration=%9u %s Duration2=%9u %s Can-Messages=%ld \n",
               (uint32_t)(time_1-start_time), TIME_UNITS,
               (uint32_t)(stop_time-time_1), TIME_UNITS, (long)len);
      }
      if (ret == NTCAN_SUCCESS) {
        if (!(pCtx->mode & NO_ASCII_OUTPUT)) {
          for (i = 0; i < len; i++) {
    ...
    case 120:
        pCtx->txmsg[0].id  = idstart;
        pCtx->txmsg[0].len = 8;

        {
            uint32_t ulTxDelayMin  = 0xFFFFFFFF;
            uint32_t ulTxDelayMax  = 0;
            uint64_t ullTxDelaySum = 0;

            for(i = 0; i < maxcount; i++) {
                ret = canIoctl(h0, NTCAN_IOCTL_GET_TIMESTAMP, &pCtx->txmsg[0].data);
                if (ret != NTCAN_SUCCESS) {
                    printf("canIoctl returned: %s (failed to acquire timestamp)\n",
                        get_error_str(str_buf, ret));
                    continue;
...
#if 0
                    printf("%I64x %I64x\n", *(uint64_t *)&pCtx->rxmsg_t->data, pCtx->rxmsg_t->timestamp);
#endif
                    ulTxDelay = (uint32_t)((pCtx->rxmsg_t->timestamp - *(uint64_t *)&pCtx->rxmsg_t->data) *
                        INT64_C(1000000) / pCtx->udtTimestampFreq); /*us*/

                    ullTxDelaySum += ulTxDelay;
                    if(ulTxDelay < ulTxDelayMin) {
                        ulTxDelayMin = ulTxDelay;
                    }
                    if (ulTxDelay > ulTxDelayMax) {
                        ulTxDelayMax = ulTxDelay;
                    }
                } else {
                    printf("canReadT returned: %s\n", get_error_str(str_buf,ret));
                }
            }

            printf("TxDelay (Min/Avg/Max) = %6lu / %6lu / %6lu us\n",
                   (long unsigned int)ulTxDelayMin,
                   (long unsigned int)(ullTxDelaySum / maxcount),
                   (long unsigned int)ulTxDelayMax);
        }
    ...
    case 124:
        ...
        for (i = 0; i < maxcount; i++) {
            ret = canIoctl(h0, NTCAN_IOCTL_GET_TIMESTAMP, &pCtx->ullLastTime);
			if (ret != NTCAN_SUCCESS) {
				printf("canIoctl returned: %s (failed to acquire timestamp)\n",
						get_error_str(str_buf, ret));
				break;
            ...
            if (lastStamp != 0) {
                if (pCtx->ullLastTime < lastStamp) {
                    printf("%08d: Act:%08x%08x Old:%08x%08x Diff: %u\n", i,
                        (uint32_t)((pCtx->ullLastTime >> 32) & 0xFFFFFFFF), (uint32_t)(pCtx->ullLastTime & 0xFFFFFFFF),
                        (uint32_t)((lastStamp >> 32) & 0xFFFFFFFF), (uint32_t)(lastStamp & 0xFFFFFFFF),
                        (uint32_t)(lastStamp - pCtx->ullLastTime));
                }
            }
            lastStamp = pCtx->ullLastTime;
        }
   ...
   */
  if (IS_SEND_TEST(test) && (NTCAN_SUCCESS == ret)) {
      SLEEP(txtout);
  }
...
static void help(int testnr)
{
  ...
  printf("CAN Test%s Rev %d.%d.%d  -- (c) 1997 - 2018 esd electronics gmbh\n\n",
#if defined(NTCAN_FD)
         " FD",
#else
         "",
      ...
      if (ret != NTCAN_SUCCESS) {
        ...
        if (ret != NTCAN_SUCCESS) {
          ...
          bitrate.valid = NTCAN_INVALID_PARAMETER;
            ...
            printf("Net %3d: ID=%s Serial no.: %s", i, boardid, cBuffer);
#ifdef NTCAN_IOCTL_GET_INFO
            if ((-3 == testnr) && (0 != info.open_handle)) {
                printf(" (OH:%d)", info.open_handle);
            }
#endif  /* of NTCAN_IOCTL_GET_INFO */
            printf("\n");

            printf("         Versions (hex): Lib=%1X.%1X.%02X"
                " Drv=%1X.%1X.%02X"
                " HW=%1X.%1X.%02X"
...
#ifdef NTCAN_IOCTL_GET_BITRATE_DETAILS
            if(NTCAN_SUCCESS == bitrate.valid) {
                printf(" (%d", (int)(bitrate.rate / 1000));
#if defined(NTCAN_FD)
                if(bitrate.rate_d != 0) {
                    printf(" / %d", (int)(bitrate.rate_d / 1000));
                }
#endif /* defined(NTCAN_FD) */
                printf(" KBit/s");
        ...
        if (-3 == testnr) {
            ...
            features & NTCAN_FEATURE_TRIPLE_SAMPLING ? '+' : '-');
        ...
        case NTCAN_TRX_MCP2561FD: printf("Microchip MCP2561FD"); break;
        case NTCAN_TRX_TCAN1051G: printf("TI TCAN1051G"); break;
        default:
            printf("Unknown (0x%02x)", info.transceiver);
        }
#endif

#if defined(NTCAN_FD) && defined(NTCAN_IOCTL_GET_FD_TDC)
        if (features & NTCAN_FEATURE_CAN_FD) {
            uint32_t tdc_config;

            ret = canIoctl(h0, NTCAN_IOCTL_GET_FD_TDC, &tdc_config);
            if (NTCAN_SUCCESS == ret) {
                printf("  TDC=");
                switch(NTCAN_GET_TDC_MODE(tdc_config))
                {
                case NTCAN_TDC_MODE_AUTO:
                    printf("Automatic (%d / %d)", NTCAN_GET_TDC_VALUE(tdc_config),
                        NTCAN_GET_TDC_VALUE(tdc_config) + (signed)NTCAN_GET_TDC_OFFSET(tdc_config));
                    break;
                case NTCAN_TDC_MODE_MANUAL:
                    printf("Manual (%d)", NTCAN_GET_TDC_VALUE(tdc_config));
                    break;
                case NTCAN_TDC_MODE_OFF:
                    printf("Disabled");
                    break;
                default:
                    printf("Undefined");
                ...
                printf(" canIoctl(IOCTL_GET_FD_TDC) error: %s", get_error_str(str_buf, ret));
            }
        }
#endif
...
static void help(int testnr)
{
  ...
  printf("CAN Test Rev %d.%d.%d  -- (c) 1997-2017 esd electronics gmbh\n\n",
         LEVEL, REVISION, CHANGE);
  printf("Available CAN-Devices: \n");
  for (i = 0; i <= NTCAN_MAX_NETS; i++) {
    ret = canOpen(i, 0, 1, 1, 0, 0, &h0);
...
static void print_event(EVMSG *e, uint64_t ts, CANTEST_CTX *pCtx)
{
    if(NTCAN_EV_CAN_ERROR == e->evid) {
        ...
        printf(" - Lost messages (Ctrl: %d, Driver: %d) - DMA Stall: %d\n",
            e->evdata.error.ctrl_overrun, e->evdata.error.fifo_overrun, e->evdata.error.dma_stall);
    }
...
#ifdef NTCAN_FD
            if(NTCAN_BAUD_FD == e->evdata.baud_change.baud) {
                printf("  New data phase baudrate : 0x%08lX", (unsigned long)e->evdata.baud_change.baud);
                printf(" (%ld baud)", (unsigned long)e->evdata.baud_change.num_baud);
            }
            else
#endif
            {
                printf("  New nominal baudrate    : 0x%08lX", (unsigned long)e->evdata.baud_change.baud);
                if ( 4 < e->len ) {
                    printf(" (%ld baud)", (unsigned long)e->evdata.baud_change.num_baud);
...
static void print_cmsg(CMSG *pCmsg, CANTEST_CTX *pCtx)
{
    ...
    /* Split CMSG 'len' into DLC and meta meta info */
    meta = ((pCmsg->len >> 4) & 0xB) | (pCmsg->id & NTCAN_20B_BASE ? 0x4 : 0x0);
    if (pCtx->mode & NTCAN_MODE_OBJECT) {
        /* No RTR / Interaction bit for object mode handle */
        meta &= ~((NTCAN_RTR | NTCAN_INTERACTION) >> 4);
    }
    len  = NTCAN_LEN_TO_DATASIZE(pCmsg->len);
    id   = NTCAN_ID(pCmsg->id);
    ...
    /* Print CAN-ID, Length and meta data */
    printf("%s=%9ld (0x%08lx) %s  len=%02X data= ",
            (pCmsg->id & NTCAN_EV_BASE) ? "EV" : "ID", (unsigned long)id,
            (unsigned long)id, pNtcanMetaData[meta], len);

    /* Return if no data is available in object mode or for RTR frames */
    if((pCtx->mode & NTCAN_MODE_OBJECT) && (pCmsg->len & NTCAN_NO_DATA)) {
        printf("NO DATA\n");
        return;
    } else if(NTCAN_IS_RTR(pCmsg->len)) {
        printf("N/A\n");
        return;
    }
    ...
    /* Decode a NTCAN event or print ASCII dump */
    if(NTCAN_IS_EVENT(pCmsg->id)) {
        printf("\n");
...
static void print_cmsg_t(CMSG_T *pCmsgT, CANTEST_CTX *pCtx)
{
    int      j;
    uint8_t  meta, len;
    uint32_t id, ulTimeDiff = 0;

    /* Split CMSG 'len' into DLC and meta meta info */
    meta = ((pCmsgT->len >> 4) & 0xB) | (pCmsgT->id & NTCAN_20B_BASE ? 0x4 : 0x0);
    if (pCtx->mode & NTCAN_MODE_OBJECT) {
        /* No RTR / Interaction bit for object mode handle */
        meta &= ~((NTCAN_RTR | NTCAN_INTERACTION) >> 4);
    }
    len  = NTCAN_LEN_TO_DATASIZE(pCmsgT->len);
    id   = NTCAN_ID(pCmsgT->id);
    ...
    if(pCtx->mode & NTCAN_MODE_OBJECT) {
        ...
        printf("ID= %4ld (0x%03lx) %s%c - %016" PRIx64 " - len=%02X  ",
            (unsigned long)id, (unsigned long)id, pNtcanMetaData[meta],
#if defined(NTCAN_FD)
            (NTCAN_ESI_FD_ERROR_PASSIVE == pCmsgT->esi ? 'P': ' '),
#else
            ' ',
#endif /* defined(NTCAN_FD) */
             pCtx->ullLastTime, len);
    } else {
        /* FIFO mode */
        if (pCtx->ullLastTime != 0) {  /* Calculate time difference */
        ...
        pCtx->ullLastTime = pCmsgT->timestamp;
        printf("%05ld.%03ld: %s=%9ld (0x%08lx) %s%c len=%02X data=",
            (unsigned long)(ulTimeDiff / 1000), (unsigned long)(ulTimeDiff % 1000),
            (pCmsgT->id & NTCAN_EV_BASE) ? "EV" : "ID", (unsigned long)id,
            (unsigned long)id, pNtcanMetaData[meta],
#if defined(NTCAN_FD)
            (NTCAN_ESI_FD_ERROR_PASSIVE == pCmsgT->esi ? 'P': ' '),
#else
            ' ',
    ...
    /* Return if no data is available in object mode or for RTR frames */
    if((pCtx->mode & NTCAN_MODE_OBJECT) && (pCmsgT->len & NTCAN_NO_DATA)) {
        printf("NO DATA\n");
        return;
    } else if(NTCAN_IS_RTR(pCmsgT->len)) {
        printf("N/A\n");
        return;
    }
    ...
    /* Decode a NTCAN event */
    if(NTCAN_IS_EVENT(pCmsgT->id)) {
        print_event((EVMSG *)pCmsgT, 0, pCtx);
    }
...
/* Print a formatted timestamped extended CAN message to stdout if      */
/*  NO_ASCII_OUTPUT flag is not set                                     */
/************************************************************************/
/************************************************************************/
#ifdef NTCAN_FD
static void print_cmsg_x(CMSG_X *pCmsgX, CANTEST_CTX *pCtx)
{
    ...
    /* Split CMSG 'len' into DLC and meta meta info */
    meta = ((pCmsgX->len >> 4) & 0xB) | (pCmsgX->id & NTCAN_20B_BASE ? 0x4 : 0x0);
    if (pCtx->mode & NTCAN_MODE_OBJECT) {
        /* No Interaction bit for object mode handle */
        meta &= ~(NTCAN_INTERACTION >> 4);
    }

    len  = NTCAN_LEN_TO_DATASIZE(pCmsgX->len);
    id   = NTCAN_ID(pCmsgX->id);
    ...
    if(pCtx->mode & NTCAN_MODE_OBJECT) {
        ...
        dpos = printf("ID= %4ld (0x%03lx) %s%c - %016" PRIx64 " - len=%02X  ",
            (unsigned long)id, (unsigned long)id, pNtcanMetaData[meta],
            (NTCAN_ESI_FD_ERROR_PASSIVE == pCmsgX->esi ? 'P': ' '),
             pCtx->ullLastTime, len);
    } else {
        /* FIFO mode */
        if (pCtx->ullLastTime != 0) {  /* Calculate time difference */
        ...
        pCtx->ullLastTime = pCmsgX->timestamp;
        dpos = printf("%05ld.%03ld: %s=%9ld (0x%08lx) %s%c len=%02X  ",
            (unsigned long)(ulTimeDiff / 1000), (unsigned long)(ulTimeDiff % 1000),
            (pCmsgX->id & NTCAN_EV_BASE) ? "EV" : "ID", (unsigned long)id,
            (unsigned long)id, pNtcanMetaData[meta],
            (NTCAN_ESI_FD_ERROR_PASSIVE == pCmsgX->esi ? 'P': ' '), len);
    }

    /* Return if no data is available in object mode or for RTR frames */
    if((pCtx->mode & NTCAN_MODE_OBJECT) && (pCmsgX->len & NTCAN_NO_DATA)) {
        printf("NO DATA\n");
        return;
    }
    else if (NTCAN_IS_RTR(pCmsgX->len)) {
        printf("N/A\n");
        return;
    }

    /* Decode a NTCAN event or print hex dump */
    if(NTCAN_IS_EVENT(pCmsgX->id)) {
        printf("\n");
        print_event((EVMSG *)pCmsgX, 0, pCtx);
    } else {
        for (j = 0; j < (int)len; j++) {
            if(!(j & 7)) {
                int k = 0;
...
# if defined (NTCAN_FD)
    ...
    pfnSetBaudrateX         = FUNCPTR_CAN_SET_BAUDRATE_X(hDll);
    pfnGetBaudrateX         = FUNCPTR_CAN_GET_BAUDRATE_X(hDll);
